...
with driver.session(database=DATABASE) as session:
    ...
    # B. Atomaritätschecks (mit Schwellenwerten)
    q_atomic = """
    MATCH (us:UserStory)-[:HAS_ACTION]->(a:Action)
    WITH us, count(DISTINCT a) AS act_count
    ...
    for r in session.run(q_atomic):
        ...
        if n in gt_by_norm:
            ...
            if cnt > 4:
                neo4j_results["non_atomic_gt4"].add(sid)
...
# Die ETL-Deduplizierung ist keine Neo4j-Klassifikationsausgabe. Diese separate
# Plausibilitätsprüfung bildet lediglich die Importinvariante auf dem Eingabekorpus
# nach: Byte-identische Texte erzeugen denselben verkürzten Hash. Die ermittelten
# IDs werden deshalb nicht mit Precision, Recall oder F1 bewertet.
etl_duplicate_story_ids = set()
for dataset, stories in ground_truth.items():
    seen_hashes = {}
    for story in stories:
        ...
        if digest in seen_hashes:
            if seen_hashes[digest] != text:
                raise RuntimeError(
                    f"MD5-Präfixkollision im Datensatz {dataset}: {story['id']}"
                )
            etl_duplicate_story_ids.add(story["id"])
        else:
            seen_hashes[digest] = text
...
full_report = {
    "missing_benefit": {
        ...
        "aqusa": {
            ...
            "uniform_schema_projection": {
                "TP": 0,
                "FP": 0,
                "FN": len(gt_mb)
            },
            "note": (
                "AQUSA v1 implementiert keinen eigenständigen Missing-Benefit-Check; "
                "die Projektion dient nicht als Leistungsmetrik."
            )
        },
    ...
    "atomicity": {
        ...
        "neo4j_threshold_gt4": calc(neo4j_results["non_atomic_gt4"], gt_na)
    },
    "incomplete_means": {
    ...
    "uniqueness": {
        ...
        "etl_invariant": {
            ...
            "metrics_applicable": False,
            "check": "byte_identical_texts_share_truncated_md5_key",
            "duplicate_story_ids": sorted(list(etl_duplicate_story_ids)),
            "duplicate_count": len(etl_duplicate_story_ids),
            "note": (
                "Technische Plausibilitätsprüfung der ETL-Deduplizierung; "
                "keine unabhängige Detektionsausgabe des Graphsystems."
            )
        }
    }
}
