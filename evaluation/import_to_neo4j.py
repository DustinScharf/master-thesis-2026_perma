...
EXTRACTOR_DIR = os.path.join(BASE_DIR, "extracted_data", EXPERIMENT_NAME)
...
WITH story, us
FOREACH (bName IN CASE
    WHEN story.Benefit IS NOT NULL AND trim(story.Benefit) <> ""
    THEN [story.Benefit] ELSE [] END |
    MERGE (b:Benefit {name: bName})
    MERGE (us)-[:HAS_BENEFIT]->(b)
)

WITH story, us
FOREACH (pName IN CASE
    WHEN story.Persona IS NOT NULL AND size(story.Persona) > 0
    THEN story.Persona ELSE [] END |
    MERGE (p:Persona {name: pName})
    MERGE (us)-[:HAS_PERSONA]->(p)
)

WITH story, us
FOREACH (aName IN CASE
    WHEN story.Action IS NOT NULL
         AND story.Action.`Primary Action` IS NOT NULL
    THEN story.Action.`Primary Action` ELSE [] END |
    MERGE (a:Action {name: aName})
    MERGE (us)-[:HAS_ACTION]->(a)
)

WITH story, us
FOREACH (eName IN CASE
    WHEN story.Entity IS NOT NULL
         AND story.Entity.`Primary Entity` IS NOT NULL
    THEN story.Entity.`Primary Entity` ELSE [] END |
...
def generate_clean_id(pid: str, text: str) -> str:
    """Erzeugt einen deterministischen, verkürzten Text-Fingerprint."""
    ...
    with driver.session(database=DATABASE_NAME) as session:
        ...
        for pid, fname_std, fname_int in target_files:
            ...
            for item in data:
                ...
                previous_text = seen_clean_ids.get(item["Clean_ID"])
                if previous_text is not None and previous_text != txt:
                    raise RuntimeError(
                        f"Kollision des verkürzten MD5-Fingerprints: {item['Clean_ID']}"
                    )
                seen_clean_ids[item["Clean_ID"]] = txt
                enriched.append(item)
                
            session.run(CYPHER_IMPORT_QUERY, stories=enriched, model_name=EXPERIMENT_NAME)
            print(
                f"[+] Datensatz {pid} ({len(enriched)} Stories aus "
                f"{os.path.basename(fpath)}) erfolgreich via Cypher importiert."
            )
            total_imported += len(enriched)

        print(
            f"\n[OK] ETL-Import abgeschlossen: Insgesamt {total_imported} "
            "User Stories in Neo4j geladen."
        )
    driver.close()
